...
class drawingCanvas():
    def __init__(self):
        ...
        self.canvas = None
        self.x1, self.y1 = 0, 0  # initial pos of pen
        self.val = 1  # To toggle b/w pen and eraser
        self.draw()
    ...
    def takeAction(self, k):
        if k == ord('c'):
            self.canvas = None  # When c is pressed clear the entire canvas
        if k == ord('e'):
            self.val = int(not self.val)  # press e to change between eraser mode and writing mode
